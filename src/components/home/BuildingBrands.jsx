...
const BuildingBrands = () => {
  ...
  const renderText = (text) => {
    return text.split(" ").map((word, index) => (
      <span key={index} className="word inline-block mr-2 text-[#575757]">
        {word}
      </span>
    ));
  };
  ...
  return (
    <div className="md:flex px-4 md:gap-[260px] md:pt-32 sm:px-6 md:px-20 max-w-[1440px] mx-auto">
      ...
      <div>
        ...
        >
          {renderText("Building brands for")}
          <br />
          {renderText("startups that")}
          <br />
          {renderText("make people smile")}
        </h1>
        <div className="md:py-16 py-8">
          <AnimatedButton label={"About us"} href={"/about"} />
        </div>
      </div>
    </div>
  );
